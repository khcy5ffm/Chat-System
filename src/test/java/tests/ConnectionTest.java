...
import server.ConnectionConstruction;
...
public class ConnectionTest {
	...
	static int REGISTERED = 1; // holds indicator value of user registration
	...
	public void setUpConnection(int connectionNum) {
		if(connectionNum == 1) {
			...
			c1 = new ConnectionConstruction(fakecs1, fakess);
			cArray.add(c1);
			c1Thread = new Thread(c1);			
		} else if (connectionNum == 2) {
			fakecs2 = new Socket();
			
			c2 = new ConnectionConstruction(fakecs2, fakess);
			cArray.add(c2);
			c2Thread = new Thread(c2);
		} else {
			fakecs3 = new Socket();
			
			c3 = new ConnectionConstruction(fakecs3, fakess);
			cArray.add(c3);
